...
Add layout templates — a named, reusable pane layout applied against a target directory supplied at
...
Templates are JSON, resolved by name from `<primaryRoot>/.cyber-mux/layouts/<name>.json` and then
`${XDG_CONFIG_HOME:-~/.config}/cyber-mux/layouts/<name>.json`, with the repo winning and `layout list`
...
New verbs: `cyber-mux layout list | show [--desugar] | validate`, which take a file as their subject
and answer with no multiplexer at all. Applying a template is not its own verb — it is `--layout` on
