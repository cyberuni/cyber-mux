...
Add `worktree acquire` — reuse a free worktree instead of always creating a fresh one. The twin of
`prune`: prune removes disposable worktrees, `acquireWorktree` / `WorktreeApi.acquire` recycles one
