...
- `--at <placement>` — one of `pane:right`, `pane:down`, `pane:float`, `tab` (default), or
  ...
  **floating pane** — one that sits above the tiled layout rather than taking a share of it, so
  ...
  and **zellij** (`new-pane --floating`) open a real one, while **wezterm**, **rmux**, **herdr**,
  **cmux**, and **otty** have no such concept and refuse it with `backend-unsupported` (exit 1)
  rather than quietly substituting a split.
