...
every other row is a linked worktree, and a `root` under your home directory is shortened to `~/…`.
Both are table-only: `--format json` carries the `linked` boolean and the absolute `root`.
