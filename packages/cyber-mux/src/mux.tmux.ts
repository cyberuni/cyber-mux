...
export const tmuxMuxAdapter: MuxAdapter = {
	...
	movePane(exec, target, destination, side) {
		if (
			exec('tmux', ['move-pane', '-d', side === 'down' ? '-v' : '-h', '-s', target.id, '-t', destination.id]) === null
		) {
	...
	breakPane(exec, target, _at) {
		const out = exec('tmux', ['break-pane', '-d', '-s', target.id, '-P', '-F', '#{pane_id} #{window_id}'])
...
function tmuxPaneLocation(exec: Exec, id: string, verb: string): OpenedPane {
	const out = exec('tmux', ['list-panes', '-a', '-F', '#{pane_id} #{window_id}'])
	const line = (out ?? '').split('\n').find((l) => l.split(' ')[0] === id)
