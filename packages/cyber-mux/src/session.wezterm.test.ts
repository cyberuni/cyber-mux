...
describe('spec:cyber-mux/mux', () => {
	describe('weztermSessionAdapter', () => {
		it('open() at pane:right splits with --right and reports the pane, resolving tab+workspace', () => {
			...
			const target = weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right' })
		...
		it('open() at pane:down splits with --bottom', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { 'split-pane': '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:down' })
		...
		it('open() at tab spawns into the current window, never --new-window', () => {
			...
			const target = weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'tab' })
		...
		it('open() at workspace passes --new-window and a fresh --workspace name, and reports it without an extra call', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { spawn: '9', list: LIST_ONE })
			const target = weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'workspace' })
		...
		it('open() at workspace uses --label as the workspace name when given, rather than minting one', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { spawn: '9', list: LIST_ONE })
			const target = weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'workspace', label: 'my-unit' })
		...
		it('open() at tab sets the tab title after birth, because spawn has no title flag at all', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { spawn: '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'tab', label: 'ledger' })
		...
		it('open() at pane:right degrades a --label to a stderr warning, since no pane-title primitive exists', () => {
			...
			try {
				const target = weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right', label: 'ledger' })
		...
		it('the ratio sign convention inverts, same direction as tmux', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { 'split-pane': '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right', ratio: 0.333 })
		...
		it('ratio omitted leaves wezterm its own even default', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { 'split-pane': '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right' })
		...
		it('from names the pane a pane:* split targets', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { 'split-pane': '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right', from: { id: '3' } })
		...
		it('env is never native — it rides in as an env prefix on the launch command', () => {
			const calls: string[][] = []
			const exec = fakeExec(calls, { 'split-pane': '9', list: LIST_ONE })
			weztermSessionAdapter.open(exec, {
				cwd: '/unit',
				at: 'pane:right',
				env: { ROLE: 'worker' },
				launch: 'claude',
		...
		it('env with no launch command warns to stderr rather than vanishing', () => {
			...
			try {
				weztermSessionAdapter.open(exec, { cwd: '/unit', at: 'pane:right', env: { ROLE: 'worker' } })
			} finally {
				process.stderr.write = original
			}
		...
		it('rename() on a tab uses set-tab-title', () => {
			const calls: string[][] = []
			weztermSessionAdapter.rename(fakeExec(calls), { id: '2' }, 'tab', 'ledger')
		...
		it('rename() on a pane throws — wezterm has no way to title a pane', () => {
			expect(() => weztermSessionAdapter.rename(fakeExec([]), { id: '9' }, 'pane', 'ledger')).toThrow(/pane/i)
		})

		it('group() is a complete no-op — the real workspace tier is already the group', () => {
			const calls: string[][] = []
			weztermSessionAdapter.group(fakeExec(calls), { id: '2' }, 'my-group', 'ledger')
			expect(calls).toEqual([])
		})

		it('sendText types literal text as a bracketed paste, pressing no Enter', () => {
			const calls: string[][] = []
			weztermSessionAdapter.sendText(fakeExec(calls), { id: '9' }, 'Enter')
		...
		it('sendKeys presses core-vocabulary keys as their raw byte sequences via --no-paste', () => {
			const calls: string[][] = []
			weztermSessionAdapter.sendKeys(fakeExec(calls), { id: '9' }, ['Up', 'Enter'])
		...
		it('a non-core token this adapter cannot encode is forwarded as its own literal characters', () => {
			const calls: string[][] = []
			weztermSessionAdapter.sendKeys(fakeExec(calls), { id: '9' }, ['Zzz'])
		...
		it('submit with text types it literally then presses Enter, two calls (no atomic primitive)', () => {
			const calls: string[][] = []
			weztermSessionAdapter.submit(fakeExec(calls), { id: '9' }, 'hello')
			expect(calls).toEqual([
		...
		it('submit with no text sends a bare Enter only', () => {
			const calls: string[][] = []
			weztermSessionAdapter.submit(fakeExec(calls), { id: '9' })
		...
		it('read passes --start-line as a negative offset for a trailing-lines capture', () => {
			...
			expect(weztermSessionAdapter.read(exec, { id: '9' }, { lines: 20 })).toBe('hello')
			expect(calls).toEqual([['cli', 'get-text', '--pane-id', '9', '--start-line', '-20']])
		})

		it('focus drives activate-pane', () => {
			const calls: string[][] = []
			weztermSessionAdapter.focus(fakeExec(calls), { id: '9' })
			expect(calls).toEqual([['cli', 'activate-pane', '--pane-id', '9']])
		})

		it('teardown kills the pane', () => {
			const calls: string[][] = []
			weztermSessionAdapter.teardown(fakeExec(calls), { id: '9' })
		...
		it('paneExists scans the live listing for the pane id', () => {
			...
			expect(weztermSessionAdapter.paneExists(exec, { id: '9' })).toBe(true)
			expect(weztermSessionAdapter.paneExists(exec, { id: '99' })).toBe(false)
		})
		...
		it('isPaneFocused always answers unknown — there is no primitive to ask', () => {
			expect(weztermSessionAdapter.isPaneFocused(fakeExec([]), { id: '9' })).toBeUndefined()
		})

		it('listPanes reports every live pane, its cwd stripped of the file:// scheme, and never a label', () => {
			const exec = fakeExec([], { list: LIST_ONE })
			expect(weztermSessionAdapter.listPanes(exec)).toEqual([{ id: '9', mux: 'wezterm', cwd: '/unit' }])
		})

		it('listPanes returns nothing when the backend cannot be read', () => {
			expect(weztermSessionAdapter.listPanes(fakeExec([]))).toEqual([])
		})

		it('declares it can size a split', () => {
			expect(weztermSessionAdapter.canSizeSplits).toBe(true)
		})
		...
		it('has no describeRegion, describeWorkspace, or worktree capability', () => {
			expect(weztermSessionAdapter.describeRegion).toBeUndefined()
			expect(weztermSessionAdapter.describeWorkspace).toBeUndefined()
			expect(weztermSessionAdapter.worktree).toBeUndefined()
		})
	})
})
