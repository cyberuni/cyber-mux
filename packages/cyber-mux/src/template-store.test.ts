...
/** A `TemplateStore` over an in-memory file map, recording every path it is asked for. */
function fakeStore(files: Record<string, string>): TemplateStore & { calls: Recorder } {
...
describe('spec:cyber-mux/template', () => {
	describe('templateDirs', () => {
		it('resolves the repo directory through the primary checkout, not the caller’s cwd', () => {
			...
			expect(templateDirs(gitExec, ENV)).toEqual({ repo: REPO_DIR, user: USER_DIR })
		})

		it('falls back to ~/.config when XDG_CONFIG_HOME is unset', () => {
			expect(templateDirs(gitExec, { HOME: '/home/u' }).user).toBe(USER_DIR)
		})
	})

	describe('resolveTemplate', () => {
		it('--file skips resolution entirely — neither templates directory is consulted', () => {
			const store = fakeStore({ './scratch/pool.json': template('pool') })
			const resolved = resolveTemplate({ file: './scratch/pool.json', store, exec: gitExec, env: ENV })
		...
		it('--file that cannot be read fails naming the path', () => {
			...
			expect(() => resolveTemplate({ file: './scratch/gone.json', store, exec: gitExec, env: ENV })).toThrow(
				/cannot read template: \.\/scratch\/gone\.json/,
			)
		})
		...
		it('a repo template shadows a user template of the same name', () => {
			...
			const resolved = resolveTemplate({ name: 'pool-4', store, exec: gitExec, env: ENV })
		...
		it('refuses a traversing name without even resolving the primary checkout', () => {
			...
			expect(() =>
				resolveTemplate({ name: '../../../etc/pwd', store: fakeStore({}), exec: recordingExec, env: ENV }),
	...
	describe('listTemplates', () => {
		it('reports each name’s source, marking the user template a repo template shadows', () => {
			...
			expect(listTemplates(store, { repo: REPO_DIR, user: USER_DIR })).toEqual([
