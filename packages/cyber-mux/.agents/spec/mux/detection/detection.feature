...
Feature: mux detection — which multiplexer, and which backend adapter
  ...
  Scenario: no backend detected errors before opening anything
    Given a caller with none of $TMUX, $HERDR_ENV, $WEZTERM_PANE, $ZELLIJ, or $CMUX_WORKSPACE_ID set
    When cyber-mux open runs
    Then it throws naming tmux/herdr/wezterm/zellij/cmux as the required backend
