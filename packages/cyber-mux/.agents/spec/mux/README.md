...
| **pane focus reporting** | tri-state focused / not-focused / unknown, and all three reachable on every backend (tmux/rmux: pane+window active & session attached; herdr: pane record `focused`; wezterm/zellij: the attached client's own pane, never a per-tab or per-layer flag that is true on several rows at once; cmux/otty: the row's focus field where it carries one); a query that can't be answered — no primitive, an unresolvable pane, an erroring query, a row carrying no focus field, or no client attached — → unknown so callers fail open, never a confident not-focused |
